...
function showPage(pageId, fromSidebar = false) {
    ...
    if (!home || !blog || !privacy || !bottomAd) return;
...
function searchDish() {
    ...
    if (!input || !filter || !container || !bottomAd) return;
    ...
    if (!foundDish && query.length < 2 && cuisine === "") {
        ...
        bottomAd.classList.add('hidden');
        return;
    } else if (!foundDish) {   // ✅ eksik süslü parantez burada düzeltildi
        container.innerHTML = `
    ...
    if (foundDish) {
        ...
        // 3) Şablona bas
        const templateEl = document.getElementById('dishDetailTemplate');
